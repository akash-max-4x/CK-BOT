...
const time = process.uptime() ,
  ...
  const fs = require('fs-extra');
   if (!fs.existsSync(__dirname +
        ...
        fs.writeFileSync(__dirname + `/Akash/UTM-Avo.ttf`, Buffer.from(getfont, "utf-8"));
      }
         if (!fs.existsSync(__dirname +
      `/Akash/phenomicon.ttf`)) {
      let getfont2 = (await axios.get(`https://github.com/hanakuUwU/font/raw/main/phenomicon.ttf`, { responseType: "arraybuffer" })).data;
      fs.writeFileSync(__dirname + `/Akash/phenomicon.ttf`, Buffer.from(getfont2, "utf-8"));
    };
  if (!fs.existsSync(__dirname +
      `/Akash/CaviarDreams.ttf`)) {
      let getfont3 = (await axios.get(`https://github.com/hanakuUwU/font/raw/main/CaviarDreams.ttf`, { responseType: "arraybuffer" })).data;
      fs.writeFileSync(__dirname + `/Akash/CaviarDreams.ttf`, Buffer.from(getfont3, "utf-8"));
    };
  ...
  const Canvas = require('canvas');
    let pathImg = __dirname + `/Akash/avatar_1111231.png`;
    let pathAva = __dirname + `/Akash/avatar_3dsc11.png`;
...
let l1 = await loadImage(pathAva);
  ...
  ctx.drawImage(l1, 800, -160, 1100, 1100);
     registerFont(__dirname + `/Akash/phenomicon.ttf`, {
      family: "phenomicon"
    });
    ctx.textAlign = "start";
    ...
    ctx.beginPath();
  ////////////////////////////////////////
   registerFont(__dirname + `/Akash/UTM-Avo.ttf`, {
      family: "UTM"
    });
    ctx.textAlign = "start";
    ctx.font = "70px UTM";
    ...
    ctx.restore();
    ctx.save();
registerFont(__dirname + `/Akash/CaviarDreams.ttf`, {
      family: "time"
    });
    ctx.textAlign = "start";
    ctx.font = "45px time";
    ctx.fillText("@" + "AKASH HASAN", 250, 515)
    ctx.fillText("@" + "AKASH HASAN", 250, 575)
