...
module.exports.run = async function ({ event, api, args }) {    
    ...
    if (!mention[0]) return api.sendMessage("Please mention 1 person.", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "🥵______নারী তার শখের পুরুষ কে অসংখ্য স্টাইলে চুদতে দেয়-(👌👈)-💯🤰", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
      }
